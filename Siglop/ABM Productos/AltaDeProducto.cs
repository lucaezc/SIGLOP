...
namespace Siglop
{
    public partial class AltaProd : Form
    {
        ...
        private void ConfirmaAlta_Click ( object sender, EventArgs e ) // CONFIRMAR ALTA
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        this.db.Ejecutar("INSERT INTO PRODUCTO (PRODUCTO_TIPO, PRODUCTO_DESCRIPCION, PRODUCTO_PRECIO_EFECTIVO, PRODUCTO_RECARGO_TARJETA, PRODUCTO_MEDIDAS, PRODUCTO_COLECCION, PRODUCTO_TIEMPO_REPOSICION, PRODUCTO_IMAGEN_PATH) VALUES " +
                        "('" + tipoProducto + "', '" + descripcion + "', '" + precioEfectivo + "', '" + recargoTarjeta + "', '" + medidas + "', '" + coleccion + "', '" + tiempoReposicion + "', '" + path + "')");

                        Decimal productoID = 0;
                        Boolean existe = this.db.Consultar("SELECT @@IDENTITY 'ID'");
                        if (existe)
                        {
                            this.db.Leer();
                            productoID = Decimal.Parse(this.db.ObtenerValor("ID"));
                        }

                        this.db.Ejecutar("INSERT INTO PRODUCTO_TALLE_COLOR (ID_PRODUCTO, ID_TALLE, ID_COLOR, PRODUCTO_STOCK) VALUES ('" + productoID + "', '" + talleID + "', '" + colorId + "', '" + stock + "')");

                        String confeccionDesc = "Confección para el producto " + descripcion;
                        this.db.Ejecutar("INSERT INTO CONFECCION_PRODUCTO (ID_PRODUCTO, CONFECCION_DESCRIPCION) VALUES ('" + productoID + "', '" + confeccionDesc + "')");
                        ...
                        foreach (var item in itemsComponentes)
                        {
                            ...
                            ConfirmaAlta.DialogResult = DialogResult.OK;
                        }
